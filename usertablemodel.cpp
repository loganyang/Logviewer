...
{
    return line.contains("User start the program");
}
bool UserTableModel::isStartStep(QString line)
{
    return line.contains("Start Step");
}
...
void UserTableModel::FilterEvent(QString EventLogs)
{
    ...
    if(log.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        while (!log.atEnd()) {
            ...
            {
                QString ProgramName = fields[4].section(" ",4,-10);
            ...
            else if(isStartStep(ReadData))
            {
                ...
                QString StepName = fields[4].section("(",0,0);
