    ...
    QwtAbstractLegend( parent )
{
    ...
    connect(d_treeView,SIGNAL(doubleClicked(const QModelIndex)),this,SLOT(handleDoubleClick(const QModelIndex)));
}
    ...
    const QList<QwtLegendData> &data )
{
    ...
    QColor col = dynamic_cast<QwtPlotCurve*>(plotItem)->pen().color();
...
//            col.setAlpha(100);
//            itemList[i]->setBackground(QBrush(col));
            itemList[i]->setForeground(QBrush(col));
        }
    }
    else
    {
    ...
    if ( item->isCheckable() )
    {
        if(item->rowCount() > 0)//Root
        {
            for ( int i = 0; i < item->rowCount(); i++ )
            {
                ...
                {
                    const qlonglong ptr = child->data().value<qlonglong>();
...
void Legend::handleDoubleClick(const QModelIndex &index )
{
    const QStandardItemModel *model =
        qobject_cast<QStandardItemModel *>( d_treeView->model() );
